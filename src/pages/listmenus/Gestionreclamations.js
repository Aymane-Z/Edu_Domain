...
const ListmenusGestionreclamationsPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/listmenus/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('listmenus/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/listmenus/edit/${data.id}`) },
			icon: "pi pi-pencil",
	...
	function IdTemplate(data){
		if(data){
			return (
				<Link to={`/listmenus/view/${data.id}`}> { data.id }</Link>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-listmenus`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="listmenus/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="listmenus/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="listmenus/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="ListmenusGestionreclamationsPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('listmenus')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/listmenus/add`}>
                            <Button label={$t('addNewListmenu')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="id_user" header={$t('idUser')}  sortable={true} ></Column>
...
ListmenusGestionreclamationsPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'listmenus',
	apiPath: 'listmenus/gestionreclamations',
	routeName: 'listmenusgestionreclamations',
...
export default ListmenusGestionreclamationsPage;
