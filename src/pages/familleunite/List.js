...
const FamilleuniteListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/familleunite/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('familleunite/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/familleunite/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('familleunite/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('familleunite/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	function CodeTemplate(data){
		if(data){
			return (
				<Link to={`/familleunite/view/${data.id}`}> { data.code }</Link>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-familleunite`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="familleunite/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="familleunite/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="familleunite/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="FamilleuniteListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('familleUnite')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/familleunite/add`}>
                            <Button label={$t('addNewFamilleUnite')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
...
FamilleuniteListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'familleunite',
	apiPath: 'familleunite/index',
	routeName: 'familleunitelist',
...
export default FamilleuniteListPage;
