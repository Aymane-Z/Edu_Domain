...
const EquipementListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/equipement/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('equipement/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/equipement/edit/${data.id}`) },
			icon: "pi pi-pencil",
	...
	function CodeTemplate(data){
		if(data){
			return (
				<Link to={`/equipement/view/${data.id}`}> { data.code }</Link>
			);
		}
	}
	function IdFamilleEquipementTemplate(data){
		if(data){
			return (
				<>{data.id_famille_equipement && <Button className="p-button-text" icon="pi pi-eye" label={data.familleequipement_code} onClick={() => app.openPageDialog(<FamilleequipementViewPage isSubPage apiPath={`/familleequipement/view/${data.id_famille_equipement}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-equipement`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="equipement/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="equipement/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="equipement/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="EquipementListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('equipement')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/equipement/add`}>
                            <Button label={$t('addNewEquipement')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="dt_acquisition" header={$t('dateAcquisition')}  sortable={true} ></Column>
                                    <Column  field="valeur" header={$t('valeur')}  sortable={true} ></Column>
                                    ...
                                    <Column  field="id_famille_equipement" header={$t('familleEquipement')} body={IdFamilleEquipementTemplate} sortable={true} ></Column>
...
EquipementListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'equipement',
	apiPath: 'equipement/index',
	routeName: 'equipementlist',
