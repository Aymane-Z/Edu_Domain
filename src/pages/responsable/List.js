...
const ResponsableListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/responsable/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('responsable/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/responsable/edit/${data.id}`) },
			icon: "pi pi-pencil",
	...
	function CodeTemplate(data){
		if(data){
			return (
				<Link to={`/responsable/view/${data.id}`}> { data.code }</Link>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-responsable`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="responsable/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="responsable/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="responsable/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="ResponsableListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('responsable')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/responsable/add`}>
                            <Button label={$t('addNewResponsable')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
...
ResponsableListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'responsable',
	apiPath: 'responsable/index',
	routeName: 'responsablelist',
...
export default ResponsableListPage;
