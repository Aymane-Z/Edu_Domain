...
import EtatuniteEditPage from 'pages/etatunite/Edit';
...
const EtatuniteViewPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('edit'),
			command: (event) => { app.openPageDialog(<EtatuniteEditPage isSubPage apiPath={`/etatunite/edit/${data.id}`} />, {closeBtn: true }) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('etatunite/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('etatunite/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	if(pageReady){
		return (
			<div>
<main id="EtatuniteViewPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container">
            <div className="grid justify-content-between align-items-center">
                ...
                <div className="col " >
                    <Title title={$t('etatUniteDetails')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
            </div>
        </div>
    </section>
                        ...
                        <div className="mb-3 grid ">
                            <div className="col-12 md:col-4">
                                <div className="card flex gap-3 align-items-center card shadow-none p-3 surface-100 ">
                                    <div className="">
                                        <div className="text-400 font-medium mb-1">{$t('etat')}</div>
                                        <div className="font-bold">{ item.etat }</div>
                                    </div>
                                </div>
                            </div>
                            <div className="col-12 md:col-4">
                            ...
                            <div className="col-12 md:col-4">
                                <div className="card flex gap-3 align-items-center card shadow-none p-3 surface-100 ">
                                    <div className="">
                                        <div className="text-400 font-medium mb-1">{$t('dateDbutEtat')}</div>
                                        <div className="font-bold">{ item.dt_debut_etat }</div>
                                    </div>
                                </div>
                            </div>
                            <div className="col-12 md:col-4">
                                <div className="card flex gap-3 align-items-center card shadow-none p-3 surface-100 ">
                                    <div className="">
                                        <div className="text-400 font-medium mb-1">{$t('dateFinEtat')}</div>
                                        <div className="font-bold">{ item.dt_fin_etat }</div>
                                    </div>
                                </div>
                            </div>
                            <div className="col-12 md:col-4">
                                <div className="card flex gap-3 align-items-center card shadow-none p-3 surface-100 ">
                                    <div className="">
                                        <div className="text-400 font-medium mb-1">{$t('idUnitelocation')}</div>
                                        <div className="font-bold">{item.id_unitelocation && <Button className="p-button-text" icon="pi pi-eye" label={$t('uniteLocationDetail')} onClick={() => app.openPageDialog(<UnitelocationViewPage isSubPage apiPath={`/unitelocation/view/${item.id_unitelocation}`} />, {closeBtn: true })} /> }</div>
                                    </div>
                                </div>
                            </div>
                            <div className="col-12 md:col-4">
...
EtatuniteViewPage.defaultProps = {
	id: null,
	primaryKey: 'id',
	pageName: 'etatunite',
	apiPath: 'etatunite/view',
	routeName: 'etatuniteview',
...
export default EtatuniteViewPage;
