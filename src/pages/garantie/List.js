...
import GarantViewPage from 'pages/garant/View';
import useApp from 'hooks/useApp';
import useAuth from 'hooks/useAuth';
import useUtils from 'hooks/useUtils';

import useListPage from 'hooks/useListPage';
const GarantieListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/garantie/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('garantie/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/garantie/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('garantie/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('garantie/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	function IdTemplate(data){
		if(data){
			return (
				<Link to={`/garantie/view/${data.id}`}> { data.id }</Link>
			);
		}
	}
	function IdGarantTemplate(data){
		if(data){
			return (
				<>{data.id_garant && <Button className="p-button-text" icon="pi pi-eye" label={data.garant_denomination} onClick={() => app.openPageDialog(<GarantViewPage isSubPage apiPath={`/garant/view/${data.id_garant}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-garantie`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="garantie/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="garantie/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="garantie/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="GarantieListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('garantie')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/garantie/add`}>
                            <Button label={$t('addNewGarantie')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="id_garant" header={$t('garant')} body={IdGarantTemplate} sortable={true} ></Column>
...
GarantieListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'garantie',
	apiPath: 'garantie/index',
	routeName: 'garantielist',
...
export default GarantieListPage;
