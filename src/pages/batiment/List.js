...
const BatimentListPage = (props) => {
	...
	const { filters, setFilterValue, setFilterOptions, filterHasValue } = filterController;
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/batiment/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('batiment/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/batiment/edit/${data.id}`) },
			icon: "pi pi-pencil",
	...
	function CodeTemplate(data){
		if(data){
			return (
				<Link to={`/batiment/view/${data.id}`}> { data.code }</Link>
			);
		}
	}
	function IdResidenceTemplate(data){
		if(data){
			return (
				<>{data.id_residence && <Button className="p-button-text" icon="pi pi-building" label={data.residence_denomination} onClick={() => app.openPageDialog(<ResidenceViewPage isSubPage apiPath={`/residence/view/${data.id_residence}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-batiment`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="batiment/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="batiment/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="batiment/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="BatimentListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('batiment')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/batiment/add`}>
                            <Button label={$t('Nouvel Bâtiment')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                            ...
                            <Dialog dismissableMask visible={Modal780} onHide={()=>setModal780(false)} breakpoints={{'960px': '75vw', '640px': '100vw'}} style={{width: '750px'}} modal  header={<>
                                ...
                                <div className="card  sp-3 mb-3">
                                    <DataSource onLoad={(options) => setFilterOptions('id_residence', options)}  apiPath="components_data/id_residence_option_list_3"  >
                                        ...
                                        ({ response }) => 
                                        <>
                                        <Title title={$t('filterParResidences')} text headerClass="p-3" titleClass="font-bold text-primary" subTitleClass="text-sm text-500" pi pi-sliders-vclass   pi-sliders-vright  separator={false} />
                                        <div className="">
                                ...
                                <div className="card  sp-3 mb-3">
                                    <Title title={$t('filterParNombreDeNiveaux')} text headerClass="p-3" titleClass="font-bold text-primary" subTitleClass="text-sm text-500" pi pi-sliders-vclass   pi-sliders-vright  separator={false} />
                                    <div className="p-fluid ">
                                        { filterHasValue('nombre_niveaux') &&  <div className="flex justify-content-between">
                                            <Chip label={`${filters.nombre_niveaux.value[0]}`} />
                                            <Chip label={`${filters.nombre_niveaux.value[1]}`} />
                                        </div>
                                        }
                                        <Slider range value={filters.nombre_niveaux.value} onChange={(e) => setFilterValue('nombre_niveaux', e.value)} min={0} max={100} step={10} />
                                    </div>
                                </div>
                            </Dialog>
                        </div>
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column expander={true} style={{ width: '3em' }} />
                                        ...
                                        <Column  field="nombre" header={$t('nombre')}  sortable={true} ></Column>
                                        <Column  field="id_residence" header={$t('residence')} body={IdResidenceTemplate} sortable={true} ></Column>
...
BatimentListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'batiment',
	apiPath: 'batiment/index',
	routeName: 'batimentlist',
...
export default BatimentListPage;
