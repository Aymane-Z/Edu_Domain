...
const UsersnodeListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/usersnode/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('usersnode/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/usersnode/edit/${data.id}`) },
			icon: "pi pi-pencil",
	...
	function UserRoleIdTemplate(data){
		if(data){
			return (
				<>{data.user_role_id && <Button className="p-button-text" icon="pi pi-eye" label={data.roles_role_name} onClick={() => app.openPageDialog(<RolesViewPage isSubPage apiPath={`/roles/view/${data.user_role_id}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	function TelephoneTemplate(data){
		if(data){
			return (
					<a className="p-button-text" href={`tel:${data.telephone}`}>{ data.telephone }</a>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-usersnode`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="usersnode/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="usersnode/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="usersnode/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="UsersnodeListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('users')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/usersnode/add`}>
                            <Button label={$t('addNewUsersnode')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                                ...
                                <div className="page-records">
                                    <DataTable 
                                        ...
                                        <Column expander={true} style={{ width: '3em' }} />
                                            ...
                                            <Column  field="email" header={$t('email')} body={EmailTemplate} sortable={true} ></Column>
                                            <Column  field="photo" header={$t('photo')} body={PhotoTemplate} sortable={true} ></Column>
                                            <Column  field="user_role_id" header={$t('userRole')} body={UserRoleIdTemplate} sortable={true} ></Column>
                                            <Column  field="civilite" header={$t('civilite')}  sortable={true} ></Column>
                                            <Column  field="nom" header={$t('nom')}  sortable={true} ></Column>
                                            <Column  field="prenom" header={$t('prenom')}  sortable={true} ></Column>
                                            <Column  field="cin" header={$t('cin')}  sortable={true} ></Column>
                                            <Column  field="nationalite" header={$t('nationalite')}  sortable={true} ></Column>
                                            <Column  field="telephone" header={$t('telephone')} body={TelephoneTemplate} sortable={true} ></Column>
                                            ...
                                            <Column  field="email_verified_at" header={$t('emailVerifiedAt')}  sortable={true} ></Column>
...
UsersnodeListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'usersnode',
	apiPath: 'usersnode/index',
	routeName: 'usersnodelist',
...
export default UsersnodeListPage;
