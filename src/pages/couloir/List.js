...
import EtageViewPage from 'pages/etage/View';
...
const CouloirListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/couloir/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('couloir/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/couloir/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('couloir/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('couloir/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	function CodeTemplate(data){
		if(data){
			return (
				<Link to={`/couloir/view/${data.id}`}> { data.code }</Link>
			);
		}
	}
	...
	function IdEtageTemplate(data){
		if(data){
			return (
				<>{data.id_etage && <Button className="p-button-text" icon="pi pi-eye" label={$t('etage')} onClick={() => app.openPageDialog(<EtageViewPage isSubPage apiPath={`/etage/view/${data.id_etage}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-couloir`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="couloir/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="couloir/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="couloir/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="CouloirListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('couloir')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/couloir/add`}>
                            <Button label={$t('addNewCouloir')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                        ...
                        <div className="card  s">
                            ...
                            <Dialog dismissableMask visible={Modal780} onHide={()=>setModal780(false)} breakpoints={{'960px': '75vw', '640px': '100vw'}} style={{width: '750px'}} modal  header={<>
                                ...
                                <div className="card  sp-3 mb-3">
                                    <Title title={$t('filterByDtMiseService')} text headerClass="p-3" titleClass="font-bold text-primary" subTitleClass="text-sm text-500" pi pi-sliders-vclass   pi-sliders-vright  separator={false} />
                                    <div className="">
                                ...
                                <div className="card  sp-3 mb-3">
                                    <DataSource onLoad={(options) => setFilterOptions('id_pavillon', options)}  apiPath="components_data/id_pavillon_option_list_3"  >
                                        {
                                        ({ response }) => 
                                        <>
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column expander={true} style={{ width: '3em' }} />
                                        ...
                                        <Column  field="nombre_entrees" header={$t('nombreEntres')}  sortable={true} ></Column>
                                        ...
                                        <Column  field="id_etage" header={$t('etage')} body={IdEtageTemplate} sortable={true} ></Column>
...
CouloirListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'couloir',
	apiPath: 'couloir/index',
	routeName: 'couloirlist',
...
export default CouloirListPage;
