...
const ProprietaireListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/proprietaire/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('proprietaire/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/proprietaire/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('proprietaire/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('proprietaire/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	function CodeTemplate(data){
		if(data){
			return (
				<Link to={`/proprietaire/view/${data.id}`}> { data.code }</Link>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-proprietaire`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="proprietaire/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="proprietaire/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="proprietaire/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="ProprietaireListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('proprietaire')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/proprietaire/add`}>
                            <Button label={$t('addNewProprietaire')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
...
ProprietaireListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'proprietaire',
	apiPath: 'proprietaire/index',
	routeName: 'proprietairelist',
...
export default ProprietaireListPage;
