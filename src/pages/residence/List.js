...
const ResidenceListPage = (props) => {
	...
	const filterSchema = {
		...
		id_responsable: {
			tagTitle: $t('responsable'),
			value: [],
			valueType: 'multiple',
			options: [],
		},
		id_proprietaire: {
			tagTitle: $t('proprietaire'),
			value: [],
			valueType: 'multiple',
			options: [],
		}
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/residence/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('residence/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/residence/edit/${data.id}`) },
			icon: "pi pi-pencil",
	...
	function CodeTemplate(data){
		if(data){
			return (
				<Link to={`/residence/view/${data.id}`}> { data.code }</Link>
			);
		}
	}
	function IdResponsableTemplate(data){
		if(data){
			return (
				<>{data.id_responsable && <Button className="p-button-text" icon="pi pi-eye" label={data.responsable_nom_prenom} onClick={() => app.openPageDialog(<ResponsableViewPage isSubPage apiPath={`/responsable/view/${data.id_responsable}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	function IdProprietaireTemplate(data){
		if(data){
			return (
				<>{data.id_proprietaire && <Button className="p-button-text" icon="pi pi-eye" label={data.proprietaire_denomination} onClick={() => app.openPageDialog(<ProprietaireViewPage isSubPage apiPath={`/proprietaire/view/${data.id_proprietaire}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-residence`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="residence/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="residence/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="residence/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="ResidenceListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('Gestion des Résidences')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/residence/add`}>
                            <Button label={$t('Nouvelle Résidence')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                    ...
                    <div className="col comp-grid" >
                        <div className="">
                            ...
                            <Dialog dismissableMask visible={Modal780} onHide={()=>setModal780(false)} breakpoints={{'960px': '75vw', '640px': '100vw'}} style={{width: '750px'}} modal  header={<>
                                ...
                                <div className="card  sp-3 mb-3">
                                    <DataSource onLoad={(options) => setFilterOptions('id_responsable', options)}  apiPath="components_data/id_responsable_option_list_2"  >
                                        {
                                        ({ response }) => 
                                        <>
                                        <Title title={$t('filterParResponsable')} text headerClass="p-3" titleClass="font-bold text-primary" subTitleClass="text-sm text-500" pi pi-sliders-vclass   pi-sliders-vright  separator={false} />
                                        <div className="">
                                            <MultiSelect className="w-full" optionLabel="label" optionValue="value" value={filters.id_responsable.value} onChange={(e) => setFilterValue('id_responsable', e.value)} options={filters.id_responsable.options} label="" >
                                            </MultiSelect>
                                        </div>
                                        </>
                                        }
                                ...
                                <div className="card  sp-3 mb-3">
                                    <DataSource onLoad={(options) => setFilterOptions('id_proprietaire', options)}  apiPath="components_data/id_proprietaire_option_list_2"  >
                                        {
                                        ({ response }) => 
                                        <>
                                        <Title title={$t('filterParProprietaire')} text headerClass="p-3" titleClass="font-bold text-primary" subTitleClass="text-sm text-500" pi pi-sliders-vclass   pi-sliders-vright  separator={false} />
                                        <div className="">
                                            <MultiSelect className="w-full" optionLabel="label" optionValue="value" value={filters.id_proprietaire.value} onChange={(e) => setFilterValue('id_proprietaire', e.value)} options={filters.id_proprietaire.options} label="" >
                                            </MultiSelect>
                                        </div>
                                        </>
                                        }
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column expander={true} style={{ width: '3em' }} />
                                        ...
                                        <Column  field="denomination" header={$t('dnomination')}  sortable={true} ></Column>
                                        <Column  field="dt_mise_service" header={$t('dateMiseEnService')}  sortable={true} ></Column>
                                        <Column  field="adresse" header={$t('adresse')}  sortable={true} ></Column>
                                        <Column  field="tel1" header={$t('tlphone1')}  sortable={true} ></Column>
                                        <Column  field="tel2" header={$t('tlphone2')}  sortable={true} ></Column>
                                        <Column  field="tel3" header={$t('tlphone3')}  sortable={true} ></Column>
                                        <Column  field="id_responsable" header={$t('responsable')} body={IdResponsableTemplate} sortable={true} ></Column>
                                        <Column  field="id_proprietaire" header={$t('proprietaire')} body={IdProprietaireTemplate} sortable={true} ></Column>
...
ResidenceListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'residence',
	apiPath: 'residence/index',
	routeName: 'residencelist',
