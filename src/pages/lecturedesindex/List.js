...
const LecturedesindexListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/lecturedesindex/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('lecturedesindex/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/lecturedesindex/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('lecturedesindex/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('lecturedesindex/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	function MoisConsommationTemplate(data){
		if(data){
			return (
				<Link to={`/lecturedesindex/view/${data.id}`}> { data.mois_consommation }</Link>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-lecturedesindex`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="lecturedesindex/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="lecturedesindex/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="lecturedesindex/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="LecturedesindexListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('lectureDesIndex')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/lecturedesindex/add`}>
                            <Button label={$t('addNewLectureDesIndex')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="mois_consommation" header={$t('moisConsommation')} body={MoisConsommationTemplate} sortable={true} ></Column>
                                    ...
                                    <Column  field="index_eau_froid" header={$t('indexEauFroid')}  sortable={true} ></Column>
                                    <Column  field="index_eau_chaud" header={$t('indexEauChaud')}  sortable={true} ></Column>
                                    <Column  field="index_electricite" header={$t('indexElectricite')}  sortable={true} ></Column>
...
LecturedesindexListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'lecturedesindex',
	apiPath: 'lecturedesindex/index',
	routeName: 'lecturedesindexlist',
...
export default LecturedesindexListPage;
