...
const ContratListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/contrat/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('contrat/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/contrat/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('contrat/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('contrat/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	function RefContratTemplate(data){
		if(data){
			return (
				<Link to={`/contrat/view/${data.id}`}> { data.ref_contrat }</Link>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-contrat`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="contrat/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="contrat/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="contrat/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="ContratListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('contrat')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/contrat/add`}>
                            <Button label={$t('addNewContrat')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column expander={true} style={{ width: '3em' }} />
                                        <Column  field="ref_contrat" header={$t('refContrat')} body={RefContratTemplate} sortable={true} ></Column>
                                        <Column  field="date" header={$t('date')}  sortable={true} ></Column>
                                        <Column  field="observation" header={$t('observation')}  sortable={true} ></Column>
                                        <Column  field="remise" header={$t('remise')}  sortable={true} ></Column>
                                        <Column  field="franchize" header={$t('franchize')}  sortable={true} ></Column>
...
ContratListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'contrat',
	apiPath: 'contrat/index',
	routeName: 'contratlist',
...
export default ContratListPage;
