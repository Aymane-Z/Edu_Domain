...
const MasterDetailPages = (props) => {
	...
	function setDetailPageFormData(){
		...
		// set  form data
		const couloirFormData = { id_etage:record?.id }
        ...
        <TabPanel header={<Title title={$t('listeDesCouloirs')}  headerClass="p-0" titleClass="text-lg font-bold"  iconClass="pi pi-th-large" avatarSize="small"    separator={false} />}>
            <CanView pagePath="couloir">
                <div className="reset-grid">
                    <CouloirListPage isSubPage  fieldName="couloir.id_etage" fieldValue={masterRecord.id} showBreadcrumbs={false} showHeader={true} showFooter={true}>
                    </CouloirListPage>
                </div>
            </CanView>
        </TabPanel>
