...
const FacturationListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/facturation/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('facturation/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/facturation/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('facturation/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('facturation/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	function IdTemplate(data){
		if(data){
			return (
				<Link to={`/facturation/view/${data.id}`}> { data.id }</Link>
			);
		}
	}
	...
	function UserIdTemplate(data){
		if(data){
			return (
				<>{data.user_id && <Button className="p-button-text" icon="pi pi-eye" label={$t('usersnode')} onClick={() => app.openPageDialog(<UsersnodeViewPage isSubPage apiPath={`/usersnode/view/${data.user_id}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-facturation`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="facturation/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="facturation/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="facturation/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="FacturationListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('facturation')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/facturation/add`}>
                            <Button label={$t('addNewFacturation')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column expander={true} style={{ width: '3em' }} />
                                        ...
                                        <Column  field="dt_facturation" header={$t('dateFacturation')}  sortable={true} ></Column>
                                        ...
                                        <Column  field="user_id" header={$t('userId')} body={UserIdTemplate} sortable={true} ></Column>
...
FacturationListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'facturation',
	apiPath: 'facturation/index',
	routeName: 'facturationlist',
...
export default FacturationListPage;
