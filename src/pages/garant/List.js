...
const GarantListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/garant/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('garant/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/garant/edit/${data.id}`) },
			icon: "pi pi-pencil",
	...
	function CodeTemplate(data){
		if(data){
			return (
				<Link to={`/garant/view/${data.id}`}> { data.code }</Link>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-garant`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="garant/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="garant/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="garant/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="GarantListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('garant')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/garant/add`}>
                            <Button label={$t('addNewGarant')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column expander={true} style={{ width: '3em' }} />
                                        ...
                                        <Column  field="gerant" header={$t('gerant')}  sortable={true} ></Column>
                                        <Column  field="cin" header={$t('nCin')}  sortable={true} ></Column>
                                        <Column  field="adresse" header={$t('adresse')}  sortable={true} ></Column>
                                        <Column  field="rc" header={$t('rc')}  sortable={true} ></Column>
                                        <Column  field="patente" header={$t('patente')}  sortable={true} ></Column>
                                        ...
                                        <Column  field="compte_bancaire" header={$t('compteBancaire')}  sortable={true} ></Column>
                                        <Column  field="profession" header={$t('profession')}  sortable={true} ></Column>
                                        <Column  field="revenus_mensuels" header={$t('revenusMensuels')}  sortable={true} ></Column>
...
GarantListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'garant',
	apiPath: 'garant/index',
	routeName: 'garantlist',
