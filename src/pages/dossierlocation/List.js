...
import ContratViewPage from 'pages/contrat/View';
...
const DossierlocationListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/dossierlocation/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('dossierlocation/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/dossierlocation/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('dossierlocation/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('dossierlocation/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	function IdTemplate(data){
		if(data){
			return (
				<Link to={`/dossierlocation/view/${data.id}`}> { data.id }</Link>
			);
		}
	}
	...
	function IdUniteLocationTemplate(data){
		if(data){
			return (
				<>{data.id_unite_location && <Button className="p-button-text" icon="pi pi-eye" label={$t('uniteLocation')} onClick={() => app.openPageDialog(<UnitelocationViewPage isSubPage apiPath={`/unitelocation/view/${data.id_unite_location}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	function IdContratTemplate(data){
		if(data){
			return (
				<>{data.id_contrat && <Button className="p-button-text" icon="pi pi-eye" label={data.contrat_ref_contrat} onClick={() => app.openPageDialog(<ContratViewPage isSubPage apiPath={`/contrat/view/${data.id_contrat}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-dossierlocation`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="dossierlocation/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="dossierlocation/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="dossierlocation/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="DossierlocationListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('dossierLocation')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/dossierlocation/add`}>
                            <Button label={$t('addNewDossierLocation')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column expander={true} style={{ width: '3em' }} />
                                        <Column  field="id" header={$t('id')} body={IdTemplate} sortable={true} ></Column>
                                        <Column  field="dt_effet" header={$t('dateDeffet')}  sortable={true} ></Column>
                                        <Column  field="id_client" header={$t('client')} body={IdClientTemplate} sortable={true} ></Column>
                                        <Column  field="id_unite_location" header={$t('uniteLocation')} body={IdUniteLocationTemplate} sortable={true} ></Column>
                                        <Column  field="id_contrat" header={$t('contrat')} body={IdContratTemplate} sortable={true} ></Column>
...
DossierlocationListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'dossierlocation',
	apiPath: 'dossierlocation/index',
	routeName: 'dossierlocationlist',
...
export default DossierlocationListPage;
