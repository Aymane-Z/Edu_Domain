...
import DossierlocationEditPage from 'pages/dossierlocation/Edit';
...
const DossierlocationViewPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function ActionButton(data){
		...
		{
			label: $t('edit'),
			command: (event) => { app.openPageDialog(<DossierlocationEditPage isSubPage apiPath={`/dossierlocation/edit/${data.id}`} />, {closeBtn: true }) },
			icon: "pi pi-pencil",
	...
	if(pageReady){
		return (
			<div>
<main id="DossierlocationViewPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container">
            <div className="grid justify-content-between align-items-center">
                ...
                <div className="col " >
                    <Title title={$t('dossierLocationDetails')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
            </div>
        </div>
    </section>
                            ...
                            <div className="col">
                                ...
                                <div className="mb-3 grid ">
                                    ...
                                    <div className="col-12 md:col-4">
                                        <div className="card flex gap-3 align-items-center card shadow-none p-3 surface-100 ">
                                            <div className="">
                                                <div className="text-400 font-medium mb-1">{$t('dateDeffet')}</div>
                                                <div className="font-bold">{ item.dt_effet }</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="col-12 md:col-4">
                                    ...
                                    <div className="col-12 md:col-4">
                                        <div className="card flex gap-3 align-items-center card shadow-none p-3 surface-100 ">
                                            <div className="">
                                                ...
                                                <div className="font-bold">{item.id_unite_location && <Button className="p-button-text" icon="pi pi-eye" label={$t('uniteLocation')} onClick={() => app.openPageDialog(<UnitelocationViewPage isSubPage apiPath={`/unitelocation/view/${item.id_unite_location}`} />, {closeBtn: true })} /> }</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="col-12 md:col-4">
                                        <div className="card flex gap-3 align-items-center card shadow-none p-3 surface-100 ">
                                            <div className="">
                                                <div className="text-400 font-medium mb-1">{$t('contrat')}</div>
                                                <div className="font-bold">{item.id_contrat && <Button className="p-button-text" icon="pi pi-eye" label={item.contrat_ref_contrat} onClick={() => app.openPageDialog(<ContratViewPage isSubPage apiPath={`/contrat/view/${item.id_contrat}`} />, {closeBtn: true })} /> }</div>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="col-12 md:col-4">
...
DossierlocationViewPage.defaultProps = {
	id: null,
	primaryKey: 'id',
	pageName: 'dossierlocation',
	apiPath: 'dossierlocation/view',
	routeName: 'dossierlocationview',
...
export default DossierlocationViewPage;
