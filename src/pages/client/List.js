...
const ClientListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/client/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('client/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/client/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('client/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('client/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	function CodeTemplate(data){
		if(data){
			return (
				<Link to={`/client/view/${data.id}`}> { data.code }</Link>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-client`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="client/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="client/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="client/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="ClientListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('client')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/client/add`}>
                            <Button label={$t('addNewClient')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column expander={true} style={{ width: '3em' }} />
                                        ...
                                        <Column  field="nom_prenom" header={$t('nomEtPrnom')}  sortable={true} ></Column>
                                        <Column  field="cin" header={$t('nCin')}  sortable={true} ></Column>
                                        <Column  field="date_naissance" header={$t('dateNaissance')}  sortable={true} ></Column>
                                        <Column  field="lieu_naissance" header={$t('lieuNaissance')}  sortable={true} ></Column>
                                        <Column  field="situation_familiale" header={$t('situationFamilale')}  sortable={true} ></Column>
                                        ...
                                        <Column  field="mail" header={$t('email')}  sortable={true} ></Column>
                                        <Column  field="etablissement" header={$t('etablissement')}  sortable={true} ></Column>
                                        <Column  field="cycle_etudes" header={$t('cycleEtudes')}  sortable={true} ></Column>
