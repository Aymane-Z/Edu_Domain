...
const BatimentdetailsListPage = (props) => {
	...
	function IdProprietaireTemplate(data){
		if(data){
			return (
				<>{data.id_proprietaire && <Button className="p-button-text" icon="pi pi-eye" label={data.proprietaire_denomination} to={`/proprietaire/view/${data.id_proprietaire}`} /> }</>
			);
		}
	}
	function IdResponsableTemplate(data){
		if(data){
			return (
				<>{data.id_responsable && <Button className="p-button-text" icon="pi pi-eye" label={data.responsable_nom_prenom} to={`/responsable/view/${data.id_responsable}`} /> }</>
			);
		}
	}
	function IdResidenceTemplate(data){
		if(data){
			return (
				<>{data.id_residence && <Button className="p-button-text" icon="pi pi-eye" label={data.residence_denomination} to={`/residence/view/${data.id_residence}`} /> }</>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-batimentdetails`;
			return (
				<div className="m-2">
	...
	return (
<main id="BatimentdetailsListPage" className="main-page">
    { (props.showHeader) && 
...
BatimentdetailsListPage.defaultProps = {
	primaryKey: '',
	pageName: 'batimentdetails',
	apiPath: 'batimentdetails/index',
	routeName: 'batimentdetailslist',
...
export default BatimentdetailsListPage;
