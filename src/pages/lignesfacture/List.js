...
const LignesfactureListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/lignesfacture/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('lignesfacture/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/lignesfacture/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('lignesfacture/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('lignesfacture/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	function IdFactureTemplate(data){
		if(data){
			return (
				<>{data.id_facture && <Button className="p-button-text" icon="pi pi-eye" label={data.facture_code} onClick={() => app.openPageDialog(<FactureViewPage isSubPage apiPath={`/facture/view/${data.id_facture}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-lignesfacture`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="lignesfacture/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="lignesfacture/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="lignesfacture/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="LignesfactureListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('lignesFacture')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/lignesfacture/add`}>
                            <Button label={$t('addNewLignesFacture')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
...
LignesfactureListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'lignesfacture',
	apiPath: 'lignesfacture/index',
	routeName: 'lignesfacturelist',
...
export default LignesfactureListPage;
