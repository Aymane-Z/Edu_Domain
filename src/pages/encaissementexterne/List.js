...
const EncaissementexterneListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/encaissementexterne/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('encaissementexterne/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/encaissementexterne/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('encaissementexterne/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('encaissementexterne/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	function IdTemplate(data){
		if(data){
			return (
				<Link to={`/encaissementexterne/view/${data.id}`}> { data.id }</Link>
			);
		}
	}
	function IdEncaissementTemplate(data){
		if(data){
			return (
				<>{data.id_encaissement && <Button className="p-button-text" icon="pi pi-eye" label={$t('encaissement')} onClick={() => app.openPageDialog(<EncaissementViewPage isSubPage apiPath={`/encaissement/view/${data.id_encaissement}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-encaissementexterne`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="encaissementexterne/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importerDesDonnes')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="encaissementexterne/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="encaissementexterne/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="EncaissementexterneListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('encaissementExterne')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/encaissementexterne/add`}>
                            <Button label={$t('addNewEncaissementExterne')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column  field="id_encaissement" header={$t('idEncaissement')} body={IdEncaissementTemplate} sortable={true} ></Column>
...
EncaissementexterneListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'encaissementexterne',
	apiPath: 'encaissementexterne/index',
	routeName: 'encaissementexternelist',
...
export default EncaissementexterneListPage;
