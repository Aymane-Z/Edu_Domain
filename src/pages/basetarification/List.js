...
const BasetarificationListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/basetarification/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('basetarification/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/basetarification/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('basetarification/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('basetarification/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	function IdResidenceTemplate(data){
		if(data){
			return (
				<>{data.id_residence && <Button className="p-button-text" icon="pi pi-eye" label={$t('residence')} onClick={() => app.openPageDialog(<ResidenceViewPage isSubPage apiPath={`/residence/view/${data.id_residence}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-basetarification`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="basetarification/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="basetarification/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="basetarification/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="BasetarificationListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('baseTarification')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/basetarification/add`}>
                            <Button label={$t('addNewBaseTarification')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column expander={true} style={{ width: '3em' }} />
                                        ...
                                        <Column  field="dt_application" header={$t('dateDapplication')}  sortable={true} ></Column>
                                        <Column  field="periodicite" header={$t('periodicit')}  sortable={true} ></Column>
                                        ...
                                        <Column  field="id_residence" header={$t('idResidence')} body={IdResidenceTemplate} sortable={true} ></Column>
...
BasetarificationListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'basetarification',
	apiPath: 'basetarification/index',
	routeName: 'basetarificationlist',
...
export default BasetarificationListPage;
