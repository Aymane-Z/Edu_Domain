...
import FacturationViewPage from 'pages/facturation/View';
...
const FactureListPage = (props) => {
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/facture/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('facture/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/facture/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('facture/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('facture/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	function CodeTemplate(data){
		if(data){
			return (
				<Link to={`/facture/view/${data.id}`}> { data.code }</Link>
			);
		}
	}
	function IdFacturationTemplate(data){
		if(data){
			return (
				<>{data.id_facturation && <Button className="p-button-text" icon="pi pi-eye" label={$t('facturation')} onClick={() => app.openPageDialog(<FacturationViewPage isSubPage apiPath={`/facturation/view/${data.id_facturation}`} />, {closeBtn: true })} /> }</>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-facture`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="facture/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="facture/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="facture/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="FactureListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('facture')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/facture/add`}>
                            <Button label={$t('addNewFacture')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column expander={true} style={{ width: '3em' }} />
                                        ...
                                        <Column  field="dt_facture" header={$t('dateFacture')}  sortable={true} ></Column>
                                        <Column  field="id_facturation" header={$t('facturation')} body={IdFacturationTemplate} sortable={true} ></Column>
...
FactureListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'facture',
	apiPath: 'facture/index',
	routeName: 'facturelist',
