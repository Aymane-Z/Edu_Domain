...
const EncaissementListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function DateTemplate(data){
		if(data){
			return (
				<Link to={`/encaissement/view/${data.id}`}> { data.date }</Link>
			);
		}
	}
	function MontantTemplate(data){
		if(data){
			return (
				<Link to={`/encaissement/view/${data.id}`}> { data.montant }</Link>
			);
		}
	}
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="encaissement/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="encaissement/delete">
		<MultiDelete />
				...
				<ExportData />
	<CanView pagePath="encaissement/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="EncaissementListPage" className="main-page">
    { (props.showHeader) && 
                            ...
                            <div className="page-records">
                                <DataTable 
                                    ...
                                    <Column expander={true} style={{ width: '3em' }} />
                                        <Column  field="date" header={$t('date')} body={DateTemplate} sortable={true} ></Column>
...
EncaissementListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'encaissement',
	apiPath: 'encaissement/index',
	routeName: 'encaissementlist',
...
export default EncaissementListPage;
