...
const EncaissementNonvalidesPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	const api = useApi();
...
const refreshData = () => {
	...
	const validatePayment = async (rowData) => {
		try {
			...
			if (updateResponse.status === 200) {
				console.log('Payment update successful', updateResponse.data);
				
				setPayments(prev => prev.map(payment => 
					payment.id === rowData.id ? { ...payment, status: "valides" } : payment
				));
			}
			...
			const sendemail = await api.post(`demandelogement/validated/${auth.userId}`);
	...
	const ValidateButton = (rowData) => {
		return (
			<Button label="Valider le paiement" 
					className="p-button-success" 
					onClick={() => validatePayment(rowData)} />
		);
	};
	return (
<main id="EncaissementNonvalidesPage" className="main-page">
    { (props.showHeader) && 
                        ...
                        <div >
                            ...
                            <div className="page-records">
                                <DataTable 
									...
									<Column headerStyle={{width: '2rem'}} headerClass="text-center" body={ValidateButton}></Column>
...
EncaissementNonvalidesPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'encaissement',
	apiPath: 'encaissement/nonvalides',
	routeName: 'encaissementnonvalides',
...
export default EncaissementNonvalidesPage;
