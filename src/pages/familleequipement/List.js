...
const FamilleequipementListPage = (props) => {
		const auth = useAuth();
	const app = useApp();
	...
	function ActionButton(data){
		...
		{
			label: $t('view'),
			command: (event) => { app.navigate(`/familleequipement/view/${data.id}`) },
			icon: "pi pi-eye",
			visible: () => auth.canView('familleequipement/view')
		},
		{
			label: $t('edit'),
			command: (event) => { app.navigate(`/familleequipement/edit/${data.id}`) },
			icon: "pi pi-pencil",
			visible: () => auth.canView('familleequipement/edit')
		},
		{
			label: $t('delete'),
			command: (event) => { deleteItem(data.id) },
			icon: "pi pi-trash",
			visible: () => auth.canView('familleequipement/delete')
		}
	]
	.filter((item) => {
		if(item.visible){
	...
	function CodeTemplate(data){
		if(data){
			return (
				<Link to={`/familleequipement/view/${data.id}`}> { data.code }</Link>
			);
		}
	}
	...
	function ExportData() {
		if (props.exportData && records.length) {
			const downloadFileName = `${utils.dateNow()}-familleequipement`;
			return (
				<div className="m-2">
	...
	function ImportData() {
		if (props.importData) {
			return (
				<div className="m-2">
					<ImportPageData label={$t('selectAFileToImport')} uploadPath="familleequipement/importdata" buttonIcon="pi pi-folder" buttonLabel={$t('importData')} onImportCompleted={(response) => {app.flashMsg('Import Data', response, 'success')}} />
				</div>
			);
		}
	}
	...
	<CanView pagePath="familleequipement/delete">
		<MultiDelete />
	</CanView>
				<ExportData />
	<CanView pagePath="familleequipement/importdata">
		<ImportData />
	</CanView>
			</div>
		);
	...
	return (
<main id="FamilleequipementListPage" className="main-page">
    { (props.showHeader) && 
    <section className="page-section mb-3" >
        <div className="container-fluid">
            <div className="grid justify-content-between align-items-center">
                <div className="col " >
                    <Title title={$t('familleEquipement')}   titleClass="text-2xl text-primary font-bold" subTitleClass="text-500"      separator={false} />
                </div>
                <div className="col-fixed " >
                    <CanView pagePath={props.apiPath}>
                        <Link to={`/familleequipement/add`}>
                            <Button label={$t('addNewFamilleEquipement')} icon="pi pi-plus" type="button" className="p-button w-full bg-primary "  />
                            </Link>
                        </CanView>
                    </div>
...
FamilleequipementListPage.defaultProps = {
	primaryKey: 'id',
	pageName: 'familleequipement',
	apiPath: 'familleequipement/index',
	routeName: 'familleequipementlist',
...
export default FamilleequipementListPage;
